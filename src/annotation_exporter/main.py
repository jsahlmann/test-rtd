...
def ending_present(string: str, extension: str):
    """
    Checks the presence of the correct extension.

    :param string: File path to be checked for correct extension.
    :type string: str 
    :param extension: Required extension.
    :type extension: str

    :return: Does the file path end with the required extension? True or False.
    :rtype: boolean
    """
    split_str = string.split(".")
    return split_str[-1] == extension
...
if __name__  ==  "__main__":
    ...
    layout = [
        ...
        [sg.Push(), sg.Text("PDF path"),
        sg.InputText(key="pdf",default_text=r"PDF/aCRF 2.0_example_new"),
        sg.FileBrowse(file_types=(("PDF", "*.pdf"),))],
        [sg.Push(), sg.Text("spreadsheet path"),
        sg.InputText(key="xlsx", default_text=r"Templates/temp"),
        sg.FileBrowse(file_types=(("Excel", "*.xlsx"),))],
        [sg.Push(), sg.Text("output folder"),
        sg.InputText(key="output", default_text=r"outputs"),
        sg.FolderBrowse()],
